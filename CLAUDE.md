...
- Último MVP entregue: `docs/MVP 3.22 — Grips de todas as entidades.md` (grips de linha, círculo, arco, elipse, polyline, retângulo, texto e spline no kernel `entityGrips.ts`; arrastar ou clicar para deixar o grip quente e então clicar, digitar `x,y`/`@dx,dy`/`@d<a`, `A` para inserir vértice e `R`/Delete para remover — polylines e pontos de ajuste —; `CadTool.claimsKeyDown` para o Delete do grip; `renderEntityGrips2D`).
- MVP anterior: `docs/MVP 3.21 — Extend, Fillet e grips da spline.md` (Extend de spline — extensão natural do último segmento ou reta pela tangente —, Fillet spline × linha/arco/círculo/elipse/spline resolvido no kernel com um undo, grips da spline na ferramenta Select — pontos de ajuste ou vértices de controle com alças suaves — e conversão Fit → Control vertices no painel).
- MVP anterior: `docs/MVP 3.20 — Spline, trim de polylines e SVG avançado.md` (entidade `spline` como cadeia de Béziers cúbicas com ajuste C2 por pontos, SplineTool, snaps, Trim/Offset/Path Array, JSON/SVG exatos; offset de elipse vira spline; Trim de retângulos e polylines por trecho e Extend de polylines abertas; importação SVG com `<use>`/`<symbol>`, CSS por classe/id/tag e unidades físicas do `viewBox`; correção da distância à elipse no parâmetro 0/2π).
- MVP anterior: `docs/MVP 3.19 — Elipse completa e SVG nativo.md` (elipse com modo Eixo/Fim, eixo maior normalizado, painel com raios e ângulos do arco editáveis, perímetro/comprimento; Trim/Extend gerais — linhas, círculos, arcos, elipses e arcos de elipse, qualquer geometria como corte/limite; Offset de arco/elipse; Fillet linha × curva; Path Array em elipse; primeiro ponto digitado em Line/Arc/Circle/Rectangle; SVG com payload JSON nativo — ida e volta exata de elipses, arcos, cotas, textos, camadas e estilos — e importador novo com transforms, `<path>` completo, `<ellipse>` e camadas do Inkscape).
...
- Próximo MVP recomendado: rodada de testes dos engenheiros; depois menu de grip (Move/Rotate/Scale/Mirror pelo grip), edição de texto no canvas (duplo clique) ou seguir o roadmap para blocos e importação DXF.
