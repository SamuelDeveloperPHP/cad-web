...
export class DimDiameterTool implements CadTool {
  readonly id = "dimDiameter";
  readonly name = "DimDiameter";
  readonly aliases = ["ddi", "dimdiameter"];

  private targetCircle: CircleEntity | null = null;

  activate(context: ToolContext): void {
    this.reset(context);
    context.showMessage("[DimDiameter] Select circle");
  }
  ...
  onPointerDown(event: ToolPointerEvent, context: ToolContext): ToolResult {
    ...
    if (this.targetCircle === null) {
      ...
      context.showMessage("[DimDiameter] Specify dimension location");
      return TOOL_RESULT_NONE;
    }
