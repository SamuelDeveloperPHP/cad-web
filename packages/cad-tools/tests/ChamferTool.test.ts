...
describe("ChamferTool", () => {
  ...
  it("previews the chamfer before confirmation", () => {
    ...
    expect(preview.entities[2]).toMatchObject({ type: "line" });
  });
  ...
  it("chamfers a corner of a rectangle exploding into lines", () => {
    const tool = new ChamferTool();
    const document = createDocument([
    ...
    expect(result.type).toBe("command");
    expect(context.commands[0]).toMatchObject({ type: "ChamferCornerCommand" });
  ...
  it("undoes corner chamfer restoring the original rectangle", () => {
    const tool = new ChamferTool();
    const document = createDocument([
      { id: "rect_a", layerId: "source", type: "rectangle", x: 0, y: 0, width: 20, height: 10 }
    ]);
  ...
  it("chamfers a corner of a closed polyline", () => {
    const tool = new ChamferTool();
    const document = createDocument([
      {
